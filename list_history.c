#include "main.h"
#include "list_history.h"
/**
 * gethistory - gets the history list
 * Return: 0 uposon success
 */
ListHistory **gethistory()
{
	static ListHistory *hlist;

	return (&hlist);
}
/**
 * sethist - set hist and value
 * @command: command
 * Return: 0 upon success
 */
int sethist(char *command)
{
	ListHistory **hlistroot = gethistory();
	ListHistory *hlist = *hlistroot;
	ListHistory *ptr = hlist, *new;

	if (hlist == NULL)
	{
		new = malloc(sizeof(ListHistory));
		if (new == NULL)
			return (-1);

		new->command = _strdup(command);
		new->next = NULL;
		*hlistroot = new;
		return (0);
	}
	while (ptr->next != NULL)
		ptr = ptr->next;

	new = malloc(sizeof(ListHistory));
	if (new == NULL)
		return (-1);
	new->command = _strdup(command);
	new->next = NULL;
	ptr->next = new;
	return (0);
}
/**
 * print_hist - prints all elements of listint
 *
 * Return: num of elements
 */
int print_hist(void)
{
	ListHistory **hlistroot = gethistory();
	ListHistory *h = *hlistroot;
	int i;
	int len, numlen;
	char *s, *num;

	i = 0;
	while (h != NULL)
	{
		len = _strlen(h->command);
		s = h->command;
		num = itos(i);
		numlen = _strlen(num);
		write(1, num, numlen);
		_putchar(' ');
		write(1, s, len);
		h = h->next;
		i++;
	}
	return (i);
}
/**
 * exit_hist - exit history and copy to file
 *
 * Return: int
 */
int exit_hist(void)
{

	int fd;
	char *file = ".simple_shell_history";
	int len;
	char *s;

	ListHistory **hlistroot = gethistory();
	ListHistory *hlist = *hlistroot;
	ListHistory *ptr = hlist;

/*
 *	file = tildeexpand(file);
 */
	fd = open(file, O_CREAT | O_RDWR, 0600);
	if (fd == -1)
		return (-1);

	while (hlist != NULL)
	{
		ptr = hlist->next;
		s = hlist->command;
		len = _strlen(s);
		write(fd, s, len);
		free(hlist->command);
		free(hlist);
		hlist = ptr;
	}

	close(fd);


	return (1);
}
